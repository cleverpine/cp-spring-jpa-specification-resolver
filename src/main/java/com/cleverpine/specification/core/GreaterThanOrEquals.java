...
/**
 * The {@link GreaterThanOrEquals} class represents a single-value specification that checks whether a given property is greater than or equal
 * to a specific value. It extends the {@link SingleValueSpecification} abstract class, which means it can handle only one value
...
public class GreaterThanOrEquals<T> extends SingleValueSpecification<T> {
    ...
    /**
     * Constructs an instance of the {@link GreaterThanOrEquals} specification with the given path, value, query context, and value converter.
     *
     ...
     */
    public GreaterThanOrEquals(String attributePath, String value, QueryContext<T> queryContext, ValueConverter valueConverter) {
    ...
    public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
        ...
        return criteriaBuilder.greaterThanOrEqualTo(criteriaExpression, getValueConverter().convertToComparable(propertyType, getValue()));
    }
